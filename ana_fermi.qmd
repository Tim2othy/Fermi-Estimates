...
Create columns for the two mean values
...
for (person in people) {
    for (type in types) {
        ...
        for (i in 1:nrow(my_data)) {
            my_data[i, error_col_name] <- calc_error(my_data$Correct_estimates[i], my_data[[col_name]][i])
        }
    }
}
```
...
error_cols <- c(
    "Timothy_estimate_error", "Timothy_average_error",
    "Markus_estimate_error", "Markus_average_error",
    "Marvin_estimate_error", "Marvin_average_error",
    "Adrian_estimate_error", "Adrian_average_error",
    "Mean_estimate_error", "Mean_average_error"
)
